...
static void fht2(FLOAT *fz, int n)
{
...
void fft_long2( FLOAT wsamp_r[BLKSIZE],
	       FLOAT energy[HBLKSIZE], 
	       int chn,short int *buffer[2])
{
 ...
 if(firstcall) {
  ...
  fft( wsamp_r, energy, 1024 );
}




void fft_short2(FLOAT wsamp[3][BLKSIZE_s],
  ...
  for ( sblock = 0; sblock < 3; sblock++ ) {
    if (chn<2) 
      for ( j = 0, k = shlen * (shoff + sblock); j < 256; j++, k++ ) 
        wsamp[sblock][j] = window_s[j]* buffer[chn][k];
    if (chn==2) 
      for ( j = 0, k = shlen * (shoff + sblock); j < 256; j++, k++ ) 
        wsamp[sblock][j] = window_s[j] *
    ...
    fft( wsamp[sblock], energy_s[sblock], 256 );
  }
