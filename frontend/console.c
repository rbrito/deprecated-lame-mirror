...
static int
Console_printf(Console_IO_t * const mfp, const char *const format, ...)
{
    va_list args;
    int     ret;

    va_start(args, format);
    ret = my_console_printing(mfp->Console_fp, format, args);
    va_end(args);
...
static int
Error_printf(Console_IO_t * const mfp, const char *const format, ...)
{
    va_list args;
    int     ret;

    va_start(args, format);
    ret = my_error_printing(mfp->Error_fp, format, args);
    va_end(args);
...
static int
Report_printf(Console_IO_t * const mfp, const char *const format, ...)
{
    va_list args;
    int     ret;
    ...
    if (mfp->Report_fp != NULL) {
        va_start(args, format);
        ret = my_report_printing(mfp->Report_fp, format, args);
        va_end(args);
...
/*    closes console which was opened with open_cosole
 */
static int
close_console(Console_IO_t * const mfp)
{
