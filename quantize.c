                    ...
                    III_scalefac_t scalefac[2][2])
{
   ...
   */
  if (gfc->mode_ext==MPG_MD_MS_LR && gfp->quality > 4) { 
    reduce_s_ch=1;
  }
 ...
 */
      ...
      if (cod_info->block_type==SHORT_TYPE) {
        min_pe_bits = (pe[gr][ch]-400)*i/12;
      } else {
        min_pe_bits = (pe[gr][ch]-300)*i/22;
      }
      ...
      do {
        ...
        outer_loop( gfp,xr[gr][ch], this_bits, noise, &l3_xmin, l3_enc[gr][ch],
                    &scalefac[gr][ch], cod_info, xfsf, ch, xrpow[1]);
      ...
      if (used_bits > bits || (reduce_s_ch && ch == 1)) {        
        if (!init_outer_loop(gfp,xr[gr][ch], xrpow[0], cod_info)) {
          ...
          outer_loop( gfp,xr[gr][ch], save_bits[gr][ch], noise,
                      &l3_xmin, l3_enc[gr][ch], &scalefac[gr][ch], cod_info,
                      xfsf, ch, xrpow[0]);
        }
        if (gfp->gtkflag) {
          set_pinfo(gfp, cod_info, &ratio[gr][ch], &scalefac[gr][ch],
