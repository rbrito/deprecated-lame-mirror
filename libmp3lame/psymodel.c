...
static void
calc_mask_index_s(lame_internal_flags const *gfc, FLOAT const *max,
                  ...
                  FLOAT(*fftenergy_s)[HBLKSIZE_s], FLOAT * eb, FLOAT * thr, int chn, int sblock)
{
    ...
    calc_mask_index_s(gfc, max, avg, mask_idx_s);
...
static void
block_type_set(lame_global_flags const *gfp, int *uselongblock, int *blocktype_d)
{
    lame_internal_flags *const gfc = gfp->internal_flags;
    int     chn;
 ...
 */
     ...
     ***************************************************************/
    block_type_set(gfp, uselongblock, blocktype_d);
...
#undef GPSYCHO_BLOCK_TYPE_DECISION
    ...
    if (fabs(gfp->interChRatio) > 0.0 && gfp->mode <= JOINT_STEREO)
        calc_interchannel_masking(gfp, gfp->interChRatio);
