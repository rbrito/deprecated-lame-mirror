...
msfix_l(lame_t gfc, int gr)
{
	 ...
	 || mr[1].thm.l[sb] > (FLOAT)1.58*mr[0].thm.l[sb])
	{
	    ...
	    thmM *= x;
	    thmS *= x;
	    x = y = gfc->ATH.l_avg[sb] * gfc->ATH.adjust[2];
	} else {
	    x = gfc->mld_l[sb] * mr[3].en.l[sb]; x = Min(thmS, x);
	    y = gfc->mld_l[sb] * mr[2].en.l[sb]; y = Min(thmM, y);
	}
	mr[2].thm.l[sb] = Max(thmM, x);
	mr[3].thm.l[sb] = Max(thmS, y);
    }
}
...
msfix_s(lame_t gfc, int gr)
{
	...
	for (sblock = 0; sblock < 3; sblock++) {
		...
		thmM *= x;
		thmS *= x;
		x = y = gfc->ATH.s_avg[sb] * gfc->ATH.adjust[2];
	    } else {
		x = gfc->mld_s[sb] * mr[3].en.s[sb][sblock]; x = Min(thmS, x);
		y = gfc->mld_s[sb] * mr[2].en.s[sb][sblock]; y = Min(thmM, y);
	    }
	    mr[2].thm.s[sb][sblock] = Max(thmM, x);
	    mr[3].thm.s[sb][sblock] = Max(thmS, y);
	}
    }
}
...
inline static int
mytruncate(void *work, FLOAT x)
{
    fi_union *fi = work;
    fi->f = x + (MAGIC_FLOAT + 0.5);
    return fi->i - MAGIC_INT - 1;
}
#else /* USE_IEEE754_HACK */
# define mytruncate(dummy, x) (int)x
...
{
    int i;
    ma_max_i1 = db2pow(I1LIMIT/16.0*10.0) + 1.0;
    ma_max_i2 = db2pow(I2LIMIT/16.0*10.0) + 1.0;
...
{
    align16 FLOAT wsamp_S[2][3][BLKSIZE_s];
...
{
    FLOAT adjATH[CBANDS];
    align16 FLOAT wsamp_L[MAX_CHANNELS][BLKSIZE];    /* fft and energy calculation   */
    FLOAT nb_1[CBANDS];
    int ch;
